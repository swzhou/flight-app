...
import sample.roo.flightapp.domain.MyFlightDod;

privileged aspect MyFlightDod_Roo_DataOnDemand {
    
    declare @type: MyFlightDod: @Component;
    
    private Random MyFlightDod.rnd = new SecureRandom();
    
    private List<Flight> MyFlightDod.data;
    
    public Flight MyFlightDod.getNewTransientFlight(int index) {
    ...
    public void MyFlightDod.setEmbeddedIdClass(Flight obj, int index) {
    ...
    public void MyFlightDod.setCreatedBy(Flight obj, int index) {
        String createdBy = "createdBy_" + index;
        obj.setCreatedBy(createdBy);
    }
    
    public void MyFlightDod.setCreatedDate(Flight obj, int index) {
        Date createdDate = new GregorianCalendar(Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH), Calendar.getInstance().get(Calendar.HOUR_OF_DAY), Calendar.getInstance().get(Calendar.MINUTE), Calendar.getInstance().get(Calendar.SECOND) + new Double(Math.random() * 1000).intValue()).getTime();
        obj.setCreatedDate(createdDate);
    }
    
    public void MyFlightDod.setDestination(Flight obj, int index) {
    ...
    public void MyFlightDod.setModifiedBy(Flight obj, int index) {
        String modifiedBy = "modifiedBy_" + index;
        obj.setModifiedBy(modifiedBy);
    }
    
    public void MyFlightDod.setModifiedDate(Flight obj, int index) {
    ...
    public Flight MyFlightDod.getSpecificFlight(int index) {
        init();
        if (index < 0) {
            index = 0;
        }
    ...
    public Flight MyFlightDod.getRandomFlight() {
        init();
    ...
    public boolean MyFlightDod.modifyFlight(Flight obj) {
        return false;
    }
    
    public void MyFlightDod.init() {
        int from = 0;
        int to = 10;
